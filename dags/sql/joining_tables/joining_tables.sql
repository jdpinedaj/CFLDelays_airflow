--! WAGON_DATA table
DROP TABLE IF EXISTS cfl.public_processed.wagon_data;
CREATE TABLE IF NOT EXISTS cfl.public_processed.wagon_data AS WITH train_wagon AS (
    SELECT --* IDTRAIN_LOT
...
-------
DROP TABLE IF EXISTS cfl.public_processed.wagon_data_unstack;
CREATE TABLE IF NOT EXISTS cfl.public_processed.wagon_data_unstack AS WITH wagon_data_to_unstack AS (
    SELECT id_train_lot_train_wagon AS idtrain_lot,
        ...
        Tare_Weight,
        Wagon_Start_Date
    FROM cfl.public_processed.wagon_data
...
DROP TABLE IF EXISTS cfl.public_processed.stations_data;
CREATE TABLE IF NOT EXISTS cfl.public_processed.stations_data AS WITH train_jalon AS (
    ...
    FROM cfl.public_processed.train_jalon
),
station AS (
    ...
    FROM cfl.public_processed.station
)
SELECT *
FROM train_jalon
    LEFT JOIN station ON train_jalon.id_gare_train_jalon = station.id_gare_station;
--* Creating station_data_stops table
DROP TABLE IF EXISTS cfl.public_processed.station_data_stops;
CREATE TABLE cfl.public_processed.station_data_stops AS WITH planned_arrival AS (
    ...
    FROM cfl.public_processed.stations_data
...
DROP TABLE IF EXISTS cfl.public_processed.station_data_agg;
CREATE TABLE cfl.public_processed.station_data_agg AS WITH aggregated_data AS (
    SELECT id_train,
    ...
    FROM (
            ...
            FROM cfl.public_processed.station_data_stops
        ) t0
    GROUP BY id_train
),
...
DROP TABLE IF EXISTS cfl.public_processed.station_data;
CREATE TABLE cfl.public_processed.station_data AS WITH my_constants (start_date, end_date) AS (
...
aggregating_station_data AS (
    ...
    FROM cfl.public_processed.station_data_stops
        LEFT JOIN cfl.public_processed.station_data_agg ON station_data_stops.id_train = station_data_agg.id_train
),
o_d AS(
    SELECT *,
        CONCAT(
...
DROP TABLE IF EXISTS cfl.public_processed.train_od;
CREATE TABLE IF NOT EXISTS cfl.public_processed.train_od AS WITH origin AS (
...
DROP TABLE IF EXISTS cfl.public_processed.train_data_initial;
CREATE TABLE cfl.public_processed.train_data_initial AS
SELECT *,
    CASE
...
FROM (
        SELECT train_lot.id_train_lot AS IDTRAIN_LOT,
            ...
            train_lot.TEU_Count AS TEU_Count,
            train_lot.Max_TEU AS Max_TEU,
        ...
        FROM cfl.public_processed.train_lot
            LEFT JOIN cfl.public_processed.train ON train_lot.id_train = cfl.public_processed.train.id_train
            LEFT JOIN cfl.public_processed.train_etape ON cfl.public_processed.train_lot.id_train_lot = cfl.public_processed.train_etape.id_train_lot
    ) t0;
--* Creating train_data_final table
DROP TABLE IF EXISTS cfl.public_processed.train_data_final;
CREATE TABLE cfl.public_processed.train_data_final AS WITH advanced_train_data AS (
    SELECT *
    FROM(
            SELECT DISTINCT ON (IDTRAIN) *
            FROM cfl.public_processed.train_data_initial
            ORDER BY IDTRAIN
        ) AS train_data_initial
        RIGHT JOIN cfl.public_processed.train_od ON train_data_initial.IDTRAIN = cfl.public_processed.train_od.id_train_origin
),
...
train_position AS(
    ...
    FROM cfl.public_processed.train_position
),
station_data_merge AS (
    SELECT id_train,
        ...
        Travel_KM_HR,
        Total_KM_HR
    FROM cfl.public_processed.station_data_agg
),
station_data_to_unstack AS (
    SELECT id_train,
        ...
        KM_Distance_Event,
        KM_HR_Event
    FROM cfl.public_processed.station_data
    ORDER BY id_train,
        Timestamp_Order
),
...
SELECT --advanced_train_data.IDTRAIN_LOT,
    ...
    advanced_train_data.TEU_Count,
    advanced_train_data.Max_TEU,
    ...
    advanced_train_data.id_train_jalon_origin,
    advanced_train_data.jalon_num_origin,
    advanced_train_data.hr_arrivee_origin,
    advanced_train_data.dht_arrivee_origin,
    advanced_train_data.actual_departure,
    advanced_train_data.planned_departure,
    advanced_train_data.depart_variance,
    advanced_train_data.id_train_origin,
    advanced_train_data.id_gare_origin,
    advanced_train_data.jalon_passage_origin,
    advanced_train_data.h_depart_ecart_origin,
    advanced_train_data.h_arrivee_ecart_origin,
    advanced_train_data.id_utilisateur_creation_origin,
    advanced_train_data.dateh_creation_origin,
    advanced_train_data.id_utilisateur_maj_origin,
    advanced_train_data.dateh_maj_origin,
    advanced_train_data.commentaire_origin,
    advanced_train_data.distance_origine_origin,
    advanced_train_data.h_arrivee_ecart1_origin,
    advanced_train_data.h_arrivee_ecart2_origin,
    advanced_train_data.h_depart_ecart1_origin,
    advanced_train_data.h_depart_ecart2_origin,
    advanced_train_data.dho_depart_origin,
    advanced_train_data.dho_arrivee_origin,
    advanced_train_data.code_uic_station_origin,
    advanced_train_data.id_gare_station_origin,
    advanced_train_data.code_station_origin,
    advanced_train_data.nom_station_origin,
    advanced_train_data.origin_station,
    advanced_train_data.origin_country,
    advanced_train_data.id_gare_parent_station_origin,
    advanced_train_data.gare_frontiere_station_origin,
    advanced_train_data.origin_latitude,
    advanced_train_data.origin_longitude,
    advanced_train_data.id_utilisateur_creation_station_origin,
    advanced_train_data.dateh_creation_station_origin,
    advanced_train_data.id_utilisateur_maj_station_origin,
    advanced_train_data.dateh_maj_station_origin,
    advanced_train_data.type_gare_station_origin,
    advanced_train_data.top_everysens_existe_station_origin,
    advanced_train_data.geojson_station_origin,
    advanced_train_data.id_train_jalon_destination,
    advanced_train_data.jalon_num_destination,
    advanced_train_data.actual_arrival,
    advanced_train_data.planned_arrival,
    advanced_train_data.arrive_variance,
    advanced_train_data.dht_depart_destination,
    advanced_train_data.dhr_depart_destination,
    advanced_train_data.id_train_destination,
    advanced_train_data.id_gare_destination,
    advanced_train_data.jalon_passage_destination,
    advanced_train_data.h_depart_ecart_destination,
    advanced_train_data.h_arrivee_ecart_destination,
    advanced_train_data.id_utilisateur_creation_destination,
    advanced_train_data.dateh_creation_destination,
    advanced_train_data.id_utilisateur_maj_destination,
    advanced_train_data.dateh_maj_destination,
    advanced_train_data.commentaire_destination,
    advanced_train_data.distance_origine_destination,
    advanced_train_data.h_arrivee_ecart1_destination,
    advanced_train_data.h_arrivee_ecart2_destination,
    advanced_train_data.h_depart_ecart1_destination,
    advanced_train_data.h_depart_ecart2_destination,
    advanced_train_data.dho_depart_destination,
    advanced_train_data.dho_arrivee_destination,
    advanced_train_data.code_uic_station_destination,
    advanced_train_data.id_gare_station_destination,
    advanced_train_data.code_station_destination,
    advanced_train_data.nom_station_destination,
    advanced_train_data.destination_station,
    advanced_train_data.destination_country,
    advanced_train_data.id_gare_parent_station_destination,
    advanced_train_data.gare_frontiere_station_destination,
    advanced_train_data.destination_latitude,
    advanced_train_data.destination_longitude,
    advanced_train_data.id_utilisateur_creation_station_destination,
    advanced_train_data.dateh_creation_station_destination,
    advanced_train_data.id_utilisateur_maj_station_destination,
    advanced_train_data.dateh_maj_station_destination,
    advanced_train_data.type_gare_station_destination,
    advanced_train_data.top_everysens_existe_station_destination,
    advanced_train_data.geojson_station_destination,
    advanced_train_data.origin_city_country,
    advanced_train_data.destination_city_country,
    advanced_train_data.od_pair,
    advanced_train_data.arc,
    advanced_train_data.depart_variance_mins,
    advanced_train_data.arrive_variance_mins,
    advanced_train_data.depart_variance_mins_pos,
    advanced_train_data.arrive_variance_mins_pos,
    advanced_train_data.late_departure_bin,
    advanced_train_data.late_arrival_bin,
    advanced_train_data.late_departure_bin_15,
    advanced_train_data.late_arrival_bin_15,
    advanced_train_data.actual_departure_bin,
    advanced_train_data.actual_arrival_bin,
    advanced_train_data.bb_departure_bin,
    advanced_train_data.planned_departure_date,
    advanced_train_data.planned_arrival_date,
    advanced_train_data.planned_departure_dow,
    advanced_train_data.planned_arrival_dow,
    advanced_train_data.depart_week_num,
    ...
    cfl.public_processed.wagon_data_unstack.*
FROM advanced_train_data
    ...
    LEFT JOIN cfl.public_processed.wagon_data_unstack ON advanced_train_data.idtrain_lot = cfl.public_processed.wagon_data_unstack.idtrain_lot
WHERE station_data_merge.id_train IS NOT NULL
    AND idtrain IS NOT NULL
    AND station_data_unstack.id_train IS NOT NULL
    AND wagon_data_unstack.idtrain_lot IS NOT NULL
    AND id_train_lot_train_wagon IS NOT NULL
    AND advanced_train_data.idtrain_lot IS NOT NULL;
---------
SELECT *
FROM cfl.public_processed.train_data_final;
--! Modifying station_data TABLE
--* Creating final station_data table
DROP TABLE IF EXISTS cfl.public_processed.station_data_final;
CREATE TABLE cfl.public_processed.station_data_final AS
SELECT cfl.public_processed.train_data_final.id_train,
    cfl.public_processed.station_data.id_gare,
    cfl.public_processed.station_data.Station_Order,
    cfl.public_processed.station_data.Station_Name,
    cfl.public_processed.station_data.Station_Country,
    cfl.public_processed.station_data.Station_Latitude,
    cfl.public_processed.station_data.Station_Longitude,
    cfl.public_processed.station_data.Actual_Arrival,
    cfl.public_processed.station_data.Actual_Departure,
    cfl.public_processed.station_data.Schedule,
    cfl.public_processed.station_data.Plan_Timestamp,
    cfl.public_processed.station_data.Timestamp_Order,
    cfl.public_processed.station_data.Depart_Variance,
    cfl.public_processed.station_data.Arrive_Variance,
    cfl.public_processed.station_data.Time_From_Prior_Planned,
    cfl.public_processed.station_data.Actual_Timestamp,
    cfl.public_processed.station_data.Time_From_Prior_Actual,
    cfl.public_processed.station_data.Depart_Variance_Mins,
    cfl.public_processed.station_data.Arrive_Variance_Mins,
    cfl.public_processed.station_data.Time_From_Prior_Plan_Mins,
    cfl.public_processed.station_data.Time_From_Prior_Actual_Mins,
    cfl.public_processed.station_data.Travel_Time_Mins,
    cfl.public_processed.station_data.Idle_Time_Mins,
    cfl.public_processed.station_data.KM_Distance_Event,
    cfl.public_processed.station_data.Cumm_Distance_KM,
    cfl.public_processed.station_data.KM_HR_Event,
    cfl.public_processed.station_data.Actual_Timestamp_Valid,
    cfl.public_processed.station_data.Actual_Timestamp_Perc,
    cfl.public_processed.station_data.Cumm_Schedule_Mins,
    cfl.public_processed.station_data.Cumm_Actual_Mins,
    cfl.public_processed.station_data.Origin_Station,
    cfl.public_processed.station_data.Destination_Station,
    cfl.public_processed.station_data.Train_Distance_KM,
    cfl.public_processed.station_data.Train_Total_Time_Mins,
    cfl.public_processed.station_data.Train_Travel_Time_Mins,
    cfl.public_processed.station_data.Train_Idle_Time_Mins,
    cfl.public_processed.station_data.Planned_Departure_Date,
    cfl.public_processed.station_data.Planned_Departure_DOW,
    cfl.public_processed.station_data.Planned_Arrival_Date,
    cfl.public_processed.station_data.Planned_Arrival_DOW,
    cfl.public_processed.station_data.Train_Depart_Variance_Mins,
    cfl.public_processed.station_data.Train_Arrive_Variance_Mins,
    cfl.public_processed.station_data.Travel_KM_HR,
    cfl.public_processed.station_data.Total_KM_HR,
    cfl.public_processed.station_data.Stage_Share_Journey_Distance,
    cfl.public_processed.station_data.Stage_Share_Journey_Time_Mins,
    cfl.public_processed.station_data.Station_City_Country,
    cfl.public_processed.station_data.OD_Pair,
    cfl.public_processed.station_data.Arc,
    cfl.public_processed.station_data.Depart_Variance_Mins_Pos,
    cfl.public_processed.station_data.Arrive_Variance_Mins_Pos,
    cfl.public_processed.station_data.Late_Departure_Bin,
    cfl.public_processed.station_data.Late_Arrival_Bin,
    cfl.public_processed.station_data.Late_Departure_Bin_15,
    cfl.public_processed.station_data.Late_Arrival_Bin_15,
    cfl.public_processed.station_data.Actual_Departure_Bin,
    cfl.public_processed.station_data.Actual_Arrival_Bin,
    cfl.public_processed.station_data.Origin_Station_Bin,
    cfl.public_processed.station_data.Destination_Station_Bin
FROM cfl.public_processed.station_data
    INNER JOIN cfl.public_processed.train_data_final ON cfl.public_processed.station_data.id_train = cfl.public_processed.train_data_final.id_train
ORDER BY id_train,
    Station_Order,
    Schedule;
--! Creating incident_data Table
DROP TABLE IF EXISTS cfl.public_processed.incident_data;
CREATE TABLE IF NOT EXISTS cfl.public_processed.incident_data AS WITH incident_concerne_filtered AS (
    SELECT id_incident,
        id_train_lot
    FROM (
            SELECT cfl.public_processed.incident_concerne.id_incident AS id_incident,
                MAX(
                    cfl.public_processed.incident_concerne.id_train_lot
                ) AS id_train_lot,
                MAX(
                    cfl.public_processed.incident_concerne.annule_trainlot
                ) AS annule_trainlot
            FROM cfl.public_processed.incident_concerne
...
incidents AS (
    SELECT id_incident,
        ...
        motif_client,
        commentaire
    FROM cfl.public_processed.incidents
)
